...
# @Version: 3.2.13, build 038
# @Release: November 25th, 2024
# @Description: Helps us get a Minecraft Paper 1.21.3 server .jar
...
##### version-controller query (1.21.3)
...
# next, we want to specifically get the downloads > application > name (paper-1.21.3-96.jar)
...
if [ -f "$_cacheFile" ]; then
    ...
    if [ "$currentVersion" = "$latestVersion" ]; then
        # Example: We made a 1.21.3 server jar before, we want to only continue if what we found is 1.21.3 as well.
