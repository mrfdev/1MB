...
# This is version 0.1.6, build 015, parsing log files from Minecraft 1.20.1, generated by Paper 1.20.1
...
## TODO
# * solve issue of userName vs USERName vs UserName giving different results
...
results_log_files=$(find "$path_logs_folder" -type f -name "*.log" -exec grep -E "id=<null>.*lost connection: Disconnected$" {} \;)
...
results_gz_files=$(find "$path_logs_folder" -type f -name "*.gz" -exec zgrep -E "id=<null>.*lost connection: Disconnected$" {} \;)
