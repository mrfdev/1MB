...
# This is version 0.1.1, build 009, parsing log files from Minecraft 1.20.1, generated by Paper 1.20.1
...
log_files=$(find "$path_logs_folder" -type f -name "*.log")
grep_result=$(grep -r "$find_playername" $log_files)

# Do the same for .gz (gzipped log files), using zgrep
gz_files=$(find "$path_logs_folder" -type f -name "*.gz")
zgrep_result=$(zgrep -r "$find_playername" $gz_files)

# echo results to output files .log and .txt for playername
echo "$grep_result" > "$results_player_logfile"
echo "$zgrep_result" >> "$results_player_logfile"
