...
_minJavaVersion=22
# use 22 for java 22.0.1 which can be used with Minecraft 1.20.6 and 1.21.1
...
# 1MB-start.sh defaults to, for example: spigot-1.21.1.jar
...
# The 'default' values are for Spigot 1.21.1,
...
# We want builds for 1.21.1, so the cached version and the current version have to both be 1.21.1
# PATCH if [ "$_cacheMcBuild" == "$_currentMcBuild" ]; then
if [ "$_minecraftVersion" == "$_currentMcBuild" ]; then
    # success, 1.21.1 == 1.21.1
...
# Ok, we know there's a new build out for Spigot for Minecraft 1.21.1,
...
# If the script didn't quit here, that means we have a new spigot build for 1.21.1 of minecraft,
