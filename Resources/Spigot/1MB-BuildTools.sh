...
#
# line 1 : Minecraft version (example: 1.21)
...
# The 'default' values are for Spigot 1.21,
...
# We want builds for 1.21, so the cached version and the current version have to both be 1.21
...
# Ok, we know there's a new build out for Spigot for Minecraft 1.21,
...
# If the script didn't quit here, that means we have a new spigot build for 1.21 of minecraft,
