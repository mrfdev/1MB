...
# @Version: 2.15.4, build 093
# @Release: April 30th, 2024
# @Description: Helps us make a Minecraft Spigot 1.20.6 server.
...
# 1MB-start.sh defaults to spigot-1.20.6.jar
...
#
# line 1 : Minecraft version (example: 1.20.6)
# line 2 : Spigot nightly build version (example: 4096)
# line 3 : BuildTools build version (example: 181)
...
# The 'default' values are for Spigot 1.20.6,
...
# We want builds for 1.20.6, so the cached version and the current version have to both be 1.20.6
# PATCH if [ "$_cacheMcBuild" == "$_currentMcBuild" ]; then
if [ "$_minecraftVersion" == "$_currentMcBuild" ]; then
    # success, 1.20.6 == 1.20.6
...
# Ok, we know there's a new build out for Spigot for Minecraft 1.20.6,
...
# If the script didn't quit here, that means we have a new spigot build for 1.20.6 of minecraft,
