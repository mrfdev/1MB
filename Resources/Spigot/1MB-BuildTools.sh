...
# @Version: 2.0, build 058
# @Release: August 12th, 2020
# @Description: Helps us get a Minecraft Spigot 1.16.2 server.
...
# 1MB-start.sh defaults to spigot-1.16.2.jar
...
# The 'default' values are for Spigot 1.16.2,
...
# We want builds for 1.16.2, so the cached version and the current version have to both be 1.16.2
# PATCH if [ "$_cacheMcBuild" == "$_currentMcBuild" ]; then
if [ "$_minecraftVersion" == "$_currentMcBuild" ]; then
    # success, 1.16.2 == 1.16.2
...
# Ok, we know there's a new build out for Spigot for Minecraft 1.16.2,
...
# If the script didn't quit here, that means we have a new spigot build for 1.16.2 of minecraft,
