...
# @Version: 2.16.4, build 099
# @Release: October 22nd, 2024
# @Description: Helps us make a Minecraft Spigot 1.21.2 server.
...
_minecraftVersion="1.21.2"
# Which version are we running?

_minJavaVersion=22
# use 22 for java 22.0.2 which can be used with Minecraft 1.20.4+ and 1.21.1 and 1.21.2
...
# 1MB-start.sh defaults to, for example: spigot-1.21.2.jar
...
#
# line 1 : Minecraft version (example: 1.21.2)
...
# The 'default' values are for Spigot 1.21.2,
...
# We want builds for 1.21.2, so the cached version and the current version have to both be 1.21.2
# PATCH if [ "$_cacheMcBuild" == "$_currentMcBuild" ]; then
if [ "$_minecraftVersion" == "$_currentMcBuild" ]; then
    # success, 1.21.2 == 1.21.2
...
# Ok, we know there's a new build out for Spigot for Minecraft 1.21.2,
...
# If the script didn't quit here, that means we have a new spigot build for 1.21.2 of minecraft,
