...
# we do not support java 7, and support for anything lower than java 16 will soon be dropped.
...
# 1MB-start.sh defaults to spigot-1.19.jar
...
#
# line 1 : Minecraft version (example: 1.19)
# line 2 : Spigot nightly build version (example: 3507)
# line 3 : BuildTools build version (example: 147)
...
# The 'default' values are for Spigot 1.19,
...
# We want builds for 1.19, so the cached version and the current version have to both be 1.19
...
if [ "$_minecraftVersion" == "$_currentMcBuild" ]; then
    ...
    _output "Comparing MC : Failure; Spigot $_currentMcBuild detected, we seem to want Minecraft $_cacheMcBuild builds. We are automatically pausing the script here to make sure you do not accidentally upgrade or downgrade $_currentMcBuild server to 1.12 or 1.19 or whatever!"
...
# Ok, we know there's a new build out for Spigot for Minecraft 1.19,
...
# If the script didn't quit here, that means we have a new spigot build for 1.19 of minecraft,
