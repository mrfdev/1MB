...
# @Version: 2.14.1, build 085
# @Release: June 13th, 2023
# @Description: Helps us make a Minecraft Spigot 1.20.1 server.
...
# use 20.0 for java 20.0.1 which can be used with Minecraft 1.19.x and 1.20.1
...
# 1MB-start.sh defaults to spigot-1.20.1.jar
...
# _javaBin="/Library/Java/JavaVirtualMachines/jdk-20.0.1.jdk/Contents/Home/bin/java"
...
#
# line 1 : Minecraft version (example: 1.20.1)
# line 2 : Spigot nightly build version (example: 3799)
# line 3 : BuildTools build version (example: 157)
...
# The 'default' values are for Spigot 1.20.1,
...
# We want builds for 1.20.1, so the cached version and the current version have to both be 1.20.1
# PATCH if [ "$_cacheMcBuild" == "$_currentMcBuild" ]; then
if [ "$_minecraftVersion" == "$_currentMcBuild" ]; then
    # success, 1.20.1 == 1.20.1
...
# Ok, we know there's a new build out for Spigot for Minecraft 1.20.1,
...
# If the script didn't quit here, that means we have a new spigot build for 1.20.1 of minecraft,
